...
/**
 * @brief Compares the two arrays to check if given file is of .wojak type.
 *
 * @param extension_temp
 * @param extension_valid (contains ".wojak")
 * @return 1
 * @return 0
 */
...
int main(int argc, char *argv[]) {
    ...
    // GETTING THE EXTENSION FILE
    char *extension_temp;
    char extension_valid[] = ".wojak";

    for (int i = 1; i < sizeof(argv[1]); i++)   {
        extension_temp[i] = getExt(argv[i]);
    }

    int c = compare(extension_temp, extension_valid);

    if (c == 0)
        return 0;
    else    {
        printf("Error: cannot open file with name %s. Please check if file is of .wojak type"
